...
- `token` (TEXT) - A secure 16-byte hex API token for authentication.
...
5. **Balance Ceiling:** An employee cannot request more days than their remaining balance.
6. **No Overlaps:** Employees cannot have overlapping `PENDING` or `APPROVED` requests. Half-day requests on the exact same date for opposite halves (e.g., morning vs. afternoon) are isolated and do not count as an overlap.
...
- `POST /leaves`:
  - Payload: `{"employee_id": int, "start_date": "YYYY-MM-DD", "end_date": "YYYY-MM-DD", "reason": "str", "half_day_start": bool (optional), "half_day_end": bool (optional)}`
...
- `POST /leaves/<id>/approve`:
  ...
  - Responses: `200 OK`, `401 Unauthorized` (missing/invalid headers), `403 Forbidden` (not a manager, self-approval), `404 Not Found`, `400 Bad Request` (already processed).
